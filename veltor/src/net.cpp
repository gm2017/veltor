...
void ThreadMapPort()
{
    ...
    /* miniupnpc 1.6 */
    int error = 0;
    devlist = upnpDiscover(2000, multicastif, minissdpdpath, 0, 0, NULL, &error);
#endif
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Veltor " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Veltor is probably already running."), addrBind.ToString().c_str());
        else
