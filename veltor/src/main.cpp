...
uint256 hashGenesisBlock("0x0000066a08258414a35dd520b504fd541fde99b5bebff6a069c438667e7f44cd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Veltor: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Veltor Signed Message:\n";
...
{
    // Veltor: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Veltor: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Veltor: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000066a08258414a35dd520b504fd541fde99b5bebff6a069c438667e7f44cd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = "Thailand rocked by eleven bombs in one day";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x50bfb93c362104e631670b960fa0210a1a54116d4d81c90565ae746465508a64"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd1, 0xd2, 0xd3, 0xdb }; // Veltor: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VeltorMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VeltorMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VeltorMiner(CWallet *pwallet)
{
    printf("VeltorMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Veltor-miner");
    ...
    try { loop {
        ...
        printf("Running VeltorMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VeltorMiner, pwallet));
}
